    ...
    <div class="row mb-4">
      <div class="col-md-12">
        <div
          ...
          role="alert"
          v-if="this.$userSolde == '0' && this.$userSoldeTest == '0'"
        >
        ...
        <div class="card">
          ...
          <div class="card-body table-responsive p-0">
            <table class="table table-hover">
              ...
              <tr v-for="code in codes.data" :key="code.id">
                ...
                >
                  <span v-if="code.time !== '' && today <= code.time">{{
                    code.time
                  }}</span>
                  <span
                    v-else-if="code.time !== '' && today > code.time"
                    style="color: #DC143C"
                  >
                    {{trans('expired')}}</span
                  >
                ...
                >
                  <input
                    type="text"
                    class="form-control"
                    @keyup="changeDays($event, code.id)"
                    :value="code.days"
                  />
                </td>
                <td
                ...
                >
                  <span
                    v-if="code.time !== '' && today <= code.time"
                    style="color: #2fa360"
                    >{{trans('activated')}}</span
                  >
                  <span
                    v-else-if="code.time !== '' && today > code.time"
                    style="color: #DC143C"
                ...
                <td v-if="code.enabled == 1">
                  <a
                    v-if="code.has_is_trial"
                    @click="editModel(code)"
                    :title="trans('edit')"
                  >
                    <i class="fa fa-pencil blue-color mr-1 mr-1" style="color: blueviolet;"></i>
                  </a>
                  <a @click="ResetMac(code.id)" :title="trans('reset')">
                    <i class="fa fa-repeat mr-1" style="color: blueviolet;"></i>
                  </a>
                  <a @click="showModalDownload(code.number)" :title="trans('download')">
                    <i class="fa fa-arrow-circle-o-down mr-1 mr-1" style="color: blueviolet"></i>
                  </a>
                  <a
                    v-show="$route.meta[0].content != '0' && code.time !== ''"
                    @click="renewModal(code.number, code.package_id)"
                    :title="trans('renew')"
                  >
                    <i class="fa fa-refresh mr-1 mr-1" style="color: blueviolet"></i>
                  </a>
                  <a
                  ...
                  <a
                    @click="DELETEActiveCode(code.id, 'disabled')"
                    :title="trans('disable')"
                  >
                    <i class="fa fa-toggle-on mr-1 mr-1" style="color: green"></i
                    >
                  </a>
                  <a
                    v-show="$userType == 'Admin' || code.is_trial == '1'"
                    @click="DELETEActiveCode(code.id, 'delete')"
                    :title="trans('delete')"
                  >
          ...
          <div class="card-footer">
            <vue-paginate-al
              :totalPage="this.codes.last_page"
      ...
      <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 v-if="editmode" class="modal-title" id="addnewLabel">
              {{trans('edit_code')}}
            </h5>
          ...
          <form
            @submit.prevent="editmode ? UpdateActiveCode() : CreateActiveCode()"
            enctype="multipart/form-data"
          >
            <div class="modal-body">

              <div class="row form-group">
                <label class="col-12 control-label form_label">{{trans('code_length')}} :</label
                >
                <div class="col-md-12">
                  <input
                    type="radio"
              ...
              <div class="row form-group">
                <label class="col-12 control-label form_label">{{trans('select_pack')}} :</label>
                ...
                <div class="col-md-12" v-if="code.is_trial == 0 || !editmode || this.$userType == 'Admin'">
                  ...
                  >
                    ...
                    <option
                      ...
                      v-for="pack in packs"
                      :key="pack.id"
                      v-if="test_active == 0  && pack.is_trial == '0'"
                    >
                      {{ pack.package_name }}
                    </option>
                  </select>
                ...
                <input
                  ...
                  v-if="pack.all_bouquets"
                  type="checkbox"
                  name="select_All"
                  @change="selectAll($event, pack)"
                  class="pr-2"
                />
                <label
                ...
                <draggable v-model="pack.all_bouquets" delay="60" delay-on-touch-only draggable=".item" class="bq_items">
              ...
              <div class="row form-group">
                <label class="col-12 control-label form_label">{{trans('code_act')}} :</label>
                ...
                <div class="col-md-2" v-show="NumStart != ''">
                  <a v-on:click="getInput()" class="btn btn-success"
                    ><i class="fa fa-refresh mr-1 mr-1" style="color: white;"></i></a
                  >
                </div>
              ...
              <div class="row form-group">
                <label class="col-12 control-label form_label">{{trans('cl_name')}} :</label>
              ...
              <div class="row form-group">
                <label class="col-12 control-label form_label">{{trans('notes')}} :</label>
            ...
            <div class="modal-footer">
              ...
              <button v-else type="submit" class="btn btn-primary" style="padding: 0.375rem 0.75rem;">
                {{trans('create')}}
              </button>
            </div>
          </form>
    ...
    >
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="row form-group">
              <label class="col-12 control-label">{{trans('select_type')}} :</label>
...
export default {
  ...
  mounted() {
    ...
    Vue.prototype.$userSolde = document
      ...
      .getAttribute("content");
    this.test_active = document
      .querySelector("meta[name='testactive']")
      .getAttribute("content");
    this.getPack();
    this.getResults();
    this.all_resellers();
  ...
  methods: {
    ...
    Renew(number, package_id) {
      axios({
        ...
        if (response.data.solde > 0 || this.$userType == "Admin") {
          Swal.fire({
            title:
              "Êtes-vous sûr de renouveler? Un point sera déduit de votre solde lors du renouvellement",
    ...
    getpage(page = 1) { 
      axios.post("code/acivecodes_all?page=" + page).then((response) => {
    ...
    ResetMac(id) {
      Swal.fire({
        ...
        if (result.value) {
          axios
            .put("ActiveCode/resetMac/" + id, this.code.mac)
            .then((response) => {
              Fire.$emit("AfterCreate");
              toast.fire({
                icon: "success",
    ...
    filter(page = 1) {
      if(this.byUser != '') {
        if(this.filterVal == 0) {
          ...
          axios
          .post("code/activecode/expired/"+this.byUser+"?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/online/"+this.byUser+"?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/almost_expired/"+this.byUser+"?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/is_trial/"+this.byUser+"?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/disabled/"+this.byUser+"?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/enabled/"+this.byUser+"?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
        ...
        if(this.filterVal == 0) {
          ...
          axios
          .post("code/activecode/expired?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/online?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/almost_expired?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/is_trial?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/disabled?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
          ...
          axios
          .post("code/activecode/enabled?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
    ...
    getResults(page = 1) {      
      if(this.byUser != '') {
        if(this.filterVal == 0) {
          axios
          .post("code/ac_byres/"+this.byUser+"?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
        ...
        if(this.filterVal == 0) {
          axios
          .post("code/acivecodes_all?page=" + page + "&query=" + this.searchQuery)
          .then((response) => {
            this.codes = response.data;
            this.onLoad = false;
          });
    ...
    onChange(value) {
      this.getPackiD(event.target.value, 0);
      if(this.pack.id == 196) {
        this.NumStart = "207";
        if (!this.editmode) {
          this.code.res = "";
      ...
      } else if(this.pack.id == 209 || event.target.value == 209) {
        this.NumStart = "720";
        if (!this.editmode) {
          this.code.res = "";
          this.code.number = "";
        }
      } else if (this.pack.is_trial) {
        this.NumStart = "160";
        if (!this.editmode) {
          this.code.res = "";
          this.code.number = "";
    ...
    generateNumber(len) {
      ...
      return Math.floor(
        Math.pow(10, test - 1) +
          Math.random() * (Math.pow(10, test) - Math.pow(10, test - 1) - 1)
      );
    },
    ...
    ShowActiveCode() {
      axios
        .post("code/acivecodes_all")
        .then(({ data }) => ((this.codes = data), (this.onLoad = false)));
    },
    //------------------------------------------CREATE FUNCTION ----------------------------------\\
    ...
    UpdateActiveCode() {
      ...
      axios
        .put("code/acivecodes/" + this.code.id, {
          len: this.code.len,
          number: this.code.number,
          mac: this.code.mac,
          pack: this.code.package_id,
    ...
    EnabledActiveCode(number) {
      axios.post("code/ActiveD/" + number).then((response) => {
        Fire.$emit("AfterCreate");
      });
    },
    ...
    DELETEActiveCode(id, type) {
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete("code/acivecodes/" + id + "/" + type)
            .then((response) => {
              if (type == "disabled") {
                Swal.fire(
                  "Disabled!",
                ...
                Swal.fire("Deleted!", "this code has been deleted.", "success");
              } else {
                Swal.fire({ html: "this code has been transferd to user.<br> login: "+response.data.login + "<br> password : "+response.data.password, title: 'Transferd!'});
              }
              Fire.$emit("AfterCreate");
            })
            .catch(() => {
